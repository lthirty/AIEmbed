...
- `viewer_server.py`
  本地调试工作台服务，负责页面托管、会话/证据/分析存储、资料导入、自动测试执行、失败 session 生成、抓拍代理和 AI 自动分析
- `webapp/*`
  本地浏览器页面，围绕会话、资料、日志、导入信息和结构化分析结果工作
- `start_ai_viewer.bat`
  双击快速启动本地页面和代理服务
- `local_data/*`
...
- 新建和执行自动测试用例
- 生成测试执行记录
- FAIL 时自动创建问题 session
- 导入客户资料
- 粘贴串口日志
- 导入文字信息、图片和文档
- 可选从 ESP32-CAM 抓拍一张证据图
- 展示最近 Session、历史分析沉淀和测试库，作为开放积累资产
- 让 AI 基于资料、日志、导入信息、历史积累和当前流程做结构化自动分析
...
2. 点击“保存 AI 设置”
3. 先看“工作台概览”，了解当前会话数、证据数、分析数和测试库规模
4. 创建一个新会话，填写会话标题、客户名称、设备型号、序号、问题类型、严重级别、当前阶段和问题现象
5. 在“Session 流程工作台”里确认当前处于哪一步：
   - `现象`
   - `分层分析`
   - `验证方法`
   - `根因`
...
6. 导入客户资料
7. 粘贴串口日志，或直接用自动串口抓取
8. 在“导入信息”栏中录入文字，并按需附带图片或文档
9. 如有需要，抓拍一张设备当前画面作为辅助证据
10. 参考右侧“开放资产库”中的历史 Session、历史分析和测试库
11. 点击“开始分析”
12. 页面会显示：
   - AI 分析关键步骤
   - 流程化引导
...
1. 在“自动测试”栏创建测试用例
2. 测试用例首版支持 `serial_expect` 步骤
3. 执行时基于当前会话里的最新串口日志做匹配判断
4. PASS 时记录到最近测试结果
5. FAIL 时除了记录测试结果，还会自动生成一个新的问题 session

当前自动创建的问题 session 会附带：

- 最新串口日志副本
- 自动生成的 `test_report` 证据

这是后续扩展到 I2C 只读和完整测试报告的基础骨架。

### 11.2.2 Session 流程工作台

当前页面已经按 `deep-research-report.md` 的主线重排为流程导向工作台：

1. `工作台概览`
   - 展示会话总数、证据条目、分析记录、测试库和最近测试执行
2. `会话`
   - 不再只记录基本信息，新增：
     - `问题类型`
     - `严重级别`
...
为对齐新的 Web Spec，当前服务除了原有 `/api/...` 路由，还新增了一组更规范的别名路由：
...
- `POST /testcase`
- `GET /testcase/list`
- `POST /testrun/execute`
- `POST /knowledge/create`
- `GET /knowledge/search`
- `POST /ai/analyze`
- `POST /ai/suggest-testcase`
- `POST /ai/missing-info`
- `GET /evidence/by-session`

这样后续如果要把前端切到 `React + Next.js + Zustand`，路由语义已经更接近正式平台。

### 11.3 设计限制

- 当前主分析链路以文本资料、日志、导入信息和附件说明为主，抓拍图片只是辅助证据
...
- 根目录 [分析内容及结果.md](</f:/05.理财/31.商品/10.正式商品化/12.AI辅助嵌入式设备/分析内容及结果.md>) 会自动刷新为当前最近更新会话的 `Log + 分析结果` 汇总
...
- 这组 `Step 1 ~ Step 4` 说明卡在大屏下采用横向排列，方便单屏展示更多引导信息
...
- 当前列表页里的文件名、资料名、图片名、标签、时间等纯展示信息会尽量压缩到同一行，减少无效占高
...
2. 必须升级版本号
3. 同步更新 `CHANGELOG.md`
4. 如果本地工作台行为变化，同步更新 `viewer_server.py` 中的 `APP_VERSION`
5. 如果涉及固件行为变化，同步更新 `src/main.cpp` 中的版本号和版本历史注释
6. 如果涉及使用方式变化，同步更新本文件
7. 提交到 git
8. 同步推送到 GitHub
